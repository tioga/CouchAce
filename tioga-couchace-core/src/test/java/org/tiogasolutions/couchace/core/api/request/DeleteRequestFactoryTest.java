...
@Test
public class DeleteRequestFactoryTest {
    ...
    public void someTests() throws Exception {
        DeleteRequestFactory factory = new DeleteRequestFactory(requestExecutor);
        factory.document("some-id", "some-revision");

        try {
            factory.document(null, "some-revision");
        ...
        try {
            factory.document("some-id", null);
            Assert.fail("Expected " + IllegalArgumentException.class.getName());
        } catch (IllegalArgumentException e) {
            Assert.assertEquals(e.getMessage(), "Argument \"revision\" is null.");
        }
    }
}
